...
exports.getStaffLists = async (req, res, next) => {
  try {
    let whereObj = {};
    if (req.body.role_id) whereObj.role_id = req.body.role_id;
    ...
    if (req.body.month) payrollObj.month = req.body.month;

    if (req.body.year) payrollObj.year = req.body.year;
    ...
    for (const staff of staffs) {
      ...
      if (isGenerated) obj.status = isGenerated.getDataValue("status");
...
exports.getBasicSalary = async (req, res, next) => {
  try {
    let sal = await Staff.findByPk(req.params.id, {
...
exports.createPayroll = async (req, res, next) => {
  try {
    ...
    if (alreadyExists) return next(new AppError("Already Generated!", 400));
