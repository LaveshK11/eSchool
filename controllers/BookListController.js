...
exports.issueBook = async (req, res, next) => {
  try {
    if (!req.body.book_id) return next(new AppError("Book is required", 404));
...
exports.returnBook = async (req, res, next) => {
  try {
    ...
    if (req.body.return_date) formattedToday = req.body.return_date;

    await BookIssue.update(
      { is_returned: true, return_date: formattedToday },
      { where: { id: req.params.id } }
    );

    let book = await BookIssue.findOne({ id: req.params.id });

    await BookList.increment("qty", {
      by: 1,
      where: { id: book.getDataValue("book_id") },
    });
