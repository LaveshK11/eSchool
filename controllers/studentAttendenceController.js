...
exports.createStudentAttendence = async (req, res, next) => {
  try {
    ...
    for (const attendence of req.body.attendences) {
      let alreadyExists = await StudentAttendence.findOne({
        where: { student_id: attendence.student_id, date: attendence.date },
      });
      ...
      if (!alreadyExists)
        ...
        await StudentAttendence.update(
          { attendence: attendence.attendence, note: attendence.note },
          {
            where: {
              student_id: attendence.student_id,
            },
